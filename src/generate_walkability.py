...
def main():
    ...
    # --- Sample SRTM DEM elevation at each walk network node via GEE ---
    print("Sampling SRTM elevation at walk network nodes via GEE...")
    import ee
    ...
    node_coords = [(G.nodes[n]["x"], G.nodes[n]["y"]) for n in node_ids_list]

    BATCH = 5000
    for batch_start in range(0, len(node_ids_list), BATCH):
        batch_ids = node_ids_list[batch_start:batch_start + BATCH]
        batch_coords = node_coords[batch_start:batch_start + BATCH]
        ee_pts = ee.FeatureCollection([
            ee.Feature(ee.Geometry.Point(lon, lat), {"nid": str(nid)})
            for nid, (lon, lat) in zip(batch_ids, batch_coords)
        ])
        sampled = dem_img.sampleRegions(
            collection=ee_pts, scale=30, geometries=False
        ).getInfo()
        for feat in sampled["features"]:
            nid_str = feat["properties"]["nid"]
            elev = feat["properties"].get("elevation")
            node_elevation[int(nid_str)] = elev if elev is not None else 0
        print(f"  Sampled {batch_start + len(batch_ids)}/{len(node_ids_list)} nodes")

    # Fill any missing nodes with 0
    for n in node_ids_list:
        if n not in node_elevation:
            node_elevation[n] = 0

    print(f"  Elevation range: {min(node_elevation.values()):.0f}–{max(node_elevation.values()):.0f} m")

    # --- Compute per-edge gradient and Tobler travel time ---
    print("Computing Tobler travel times per edge...")
    for u, v, key, data in G_proj.edges(data=True, keys=True):
        length = data.get("length", 1)
        elev_u = node_elevation.get(u, 0)
        elev_v = node_elevation.get(v, 0)
        gradient = (elev_v - elev_u) / length if length > 0 else 0
        # Tobler's hiking function: speed (km/h) = 6 * exp(-3.5 * |slope + 0.05|)
        speed_kmh = 6.0 * math.exp(-3.5 * abs(gradient + 0.05))
        speed_ms = speed_kmh * 1000.0 / 3600.0  # m/s
        tobler_time = length / speed_ms if speed_ms > 0 else length / 1.0
        data["tobler_time"] = tobler_time
        data["gradient"] = round(gradient, 4)
    ...
    # 6b. Slope-adjusted Dijkstra (Tobler travel time)
    print("\nRunning slope-adjusted shortest-path computations (Tobler)...")
    distances_slope = {}
    for cat, sources in poi_nodes.items():
        if not sources:
            distances_slope[cat] = {}
        ...
        if not proj_sources:
            ...
            continue
        dist_dict = nx.multi_source_dijkstra_path_length(
            G_proj.to_undirected(), proj_sources, weight="tobler_time"
        )
        distances_slope[cat] = dist_dict
        print(f"  {cat}: computed slope-adjusted times for {len(dist_dict)} nodes")
    ...
    # 7b. Slope-adjusted transit access (Tobler time)
    for sz in subzones:
        nodes = sz["node_ids"]
        if not nodes or not distances_slope.get("mrt_exits"):
            sz["transit_access_slope"] = None
            continue
        times = [distances_slope["mrt_exits"][n] for n in nodes if n in distances_slope["mrt_exits"]]
        if times:
            median_time = statistics.median(times)
            # Invert: higher = better access (use 600s = 10 min as reference)
            sz["transit_access_slope"] = round(600.0 / median_time, 4) if median_time > 0 else None
        else:
            sz["transit_access_slope"] = None

    # 7c. Slope-adjusted destination accessibility (Tobler time)
    for sz in subzones:
        nodes = sz["node_ids"]
        if not nodes:
            sz["dest_access_slope"] = None
            continue
        weighted_sum = 0.0
        total_weight = 0
        for cat, info in POI_LAYERS.items():
            if not distances_slope.get(cat):
                continue
            times = [distances_slope[cat][n] for n in nodes if n in distances_slope[cat]]
            if not times:
                continue
            median_time = statistics.median(times)
            if median_time > 0:
                weighted_sum += info["weight"] * (600.0 / median_time)
                total_weight += info["weight"]
        sz["dest_access_slope"] = round(weighted_sum / total_weight, 4) if total_weight > 0 else None
    ...
    # 9b. Z-score composite for slope-adjusted walkability
    print("Computing slope-adjusted z-score composite...")
    slope_components = ["population_density", "intersection_density",
                        "transit_access_slope", "dest_access_slope"]
    valid_slope = [sz for sz in subzones if all(
        sz.get(c) is not None for c in slope_components
    )]
    if len(valid_slope) >= 2:
        for comp in slope_components:
            vals = [sz[comp] for sz in valid_slope]
            mean = statistics.mean(vals)
            stdev = statistics.stdev(vals) if len(vals) > 1 else 1
            for sz in valid_slope:
                sz[f"{comp}_sz"] = (sz[comp] - mean) / stdev if stdev > 0 else 0
        for sz in valid_slope:
            sz["walkability_slope"] = round(
                sum(sz[f"{c}_sz"] for c in slope_components), 4
            )
    for sz in subzones:
        if sz not in valid_slope:
            sz["walkability_slope"] = None
    ...
    for sz in subzones:
        output.append({
            ...
            "transit_access_slope": sz.get("transit_access_slope"),
            "dest_access_slope": sz.get("dest_access_slope"),
            "walkability_slope": sz.get("walkability_slope"),
        })
    ...
    for n in all_node_ids:
        ...
        # Slope-adjusted transit (Tobler time)
        t = distances_slope.get("mrt_exits", {}).get(n)
        if t is not None and t > 0:
            node_transit_slope[n] = round(600.0 / t, 4)
        ws, tw = 0.0, 0
        ws_s, tw_s = 0.0, 0
        for cat, info in POI_LAYERS.items():
            ...
            if d is not None and d > 0:
                ...
                tw += info["weight"]
            t = distances_slope.get(cat, {}).get(n)
            if t is not None and t > 0:
                ws_s += info["weight"] * (600.0 / t)
                tw_s += info["weight"]
        if tw > 0:
            node_dest[n] = round(ws / tw, 4)
        if tw_s > 0:
            node_dest_slope[n] = round(ws_s / tw_s, 4)
    ...
    while lat < NORTH:
        ...
        while lng < EAST:
            ...
            # Slope-adjusted values from nearest node
            transit_slope = node_transit_slope.get(nearest, 0)
            dest_slope = node_dest_slope.get(nearest, 0)
            ...
            grid_features.append({
                ...
                "properties": {
                    ...
                    "transit_slope": transit_slope,
                    "dest_slope": dest_slope,
                },
    ...
    # Z-score composite for slope-adjusted walkability (grid)
    slope_grid_components = ["pop_density", "int_density", "transit_slope", "dest_slope"]
    for comp in slope_grid_components:
        vals = [f["properties"][comp] for f in grid_features]
        nonzero = [v for v in vals if v > 0]
        if len(nonzero) >= 2:
            m = statistics.mean(vals)
            s = statistics.stdev(vals) if len(vals) > 1 else 1
            for f in grid_features:
                f["properties"][f"_{comp}_sz"] = (f["properties"][comp] - m) / s if s > 0 else 0
        else:
            for f in grid_features:
                f["properties"][f"_{comp}_sz"] = 0

    for f in grid_features:
        p = f["properties"]
        p["slope_walkability"] = round(
            sum(p.get(f"_{c}_sz", 0) for c in slope_grid_components), 4
        )
        for c in slope_grid_components:
            del p[f"_{c}_sz"]
    ...
    try:
        # Copy slope attributes from projected graph to unprojected for export
        for u, v, key, data in G_proj.edges(data=True, keys=True):
            if G.has_edge(u, v, key):
                G[u][v][key]["elevation_start"] = data.get("elevation_start", 0)
                G[u][v][key]["gradient"] = data.get("gradient", 0)
                G[u][v][key]["tobler_time"] = round(data.get("tobler_time", 0), 1)
        edges = ox.convert.graph_to_gdfs(G, nodes=False)
        keep_cols = ["geometry", "highway", "name", "length", "elevation_start", "gradient", "tobler_time"]
    ...
    print(f"{'Subzone':<25} {'IntDens':>7} {'Transit':>8} {'DestAcc':>8} {'WalkIdx':>8} {'TrSlope':>8} {'DsSlope':>8} {'SlopeWk':>8}")
    print("-" * 100)
    for sz in sorted(subzones, key=lambda s: s.get("walkability_index") or -999, reverse=True):
        ...
        print(f"{sz['name']:<25} {fmt(sz.get('intersection_density'))}"
              f" {fmt(sz.get('transit_access_score'))}"
              f" {fmt(sz.get('destination_accessibility'))}"
              f" {fmt(sz.get('walkability_index'))}"
              f" {fmt(sz.get('transit_access_slope'))}"
              f" {fmt(sz.get('dest_access_slope'))}"
              f" {fmt(sz.get('walkability_slope'))}")
