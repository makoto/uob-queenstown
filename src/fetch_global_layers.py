...
def generate_rs_grid():
    ...
    stack = ndvi.addBands(ndbi).addBands(lst_celsius).addBands(canopy).addBands(dem).addBands(slope)
    ...
    for k, (cell, result) in enumerate(zip(cells, all_results)):
        ...
        out_features.append({
            ...
            "properties": {
                ...
                "dem_m": round(props.get("dem") or 0, 1) if props.get("dem") is not None else None,
                "slope_deg": round(props.get("slope_deg") or 0, 2) if props.get("slope_deg") is not None else None,
            },
